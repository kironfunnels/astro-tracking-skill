...
`templates/core/tests/tracking.spec.ts` cobre, sem tocar em nenhuma plataforma real (scripts de fornecedores viram stubs vazios e `/api/events` é interceptado):
...
- relay em subdomínio responde CORS só para origens permitidas; IP/geo lidos dos cabeçalhos de Cloudflare e Vercel;
- com `spa: true`, troca de rota sem recarregar gera novo PageView (e o mesmo URL não gera);
