...
function sendToServer(event: TrackedEvent) {
  ...
  // text/plain keeps the beacon a "simple" request, so a relay on another subdomain needs no CORS preflight.
  const queued = typeof navigator.sendBeacon === 'function' && navigator.sendBeacon(tracking.endpoint, new Blob([body], { type: 'text/plain;charset=UTF-8' }));
  if (!queued) {
    fetch(tracking.endpoint, { method: 'POST', headers: { 'Content-Type': 'text/plain;charset=UTF-8' }, body, keepalive: true, credentials: 'include' }).catch(() => {});
  }
}
...
/** Thresholds already sent on the current page (reset on client-side navigation). */
const scrollSent = new Set<number>();

function trackScroll() {
  if (!tracking.scroll.length) return;
  const sent = scrollSent;
  let frame = 0;
  const check = () => {
    frame = 0;
...
/** Client-side routing: a URL change without reload is a new page for every platform. */
function watchNavigation() {
  if (!tracking.spa) return;
