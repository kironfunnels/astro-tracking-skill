...
// Stack fingerprints from the document HTML and response headers (guides which templates to use).
const STACK_SIGNATURES = [
  ['Astro', /\/_astro\/|data-astro-|<meta name="generator" content="Astro/i],
  ['Next.js', /\/_next\/static|__NEXT_DATA__|self\.__next_f/],
  ['Nuxt', /\/_nuxt\/|__NUXT__|data-nuxt/],
  ...
  ['Vite SPA', /<div id="(root|app)"><\/div>\s*<script type="module" (crossorigin )?src="\/assets\//],
];

function detectStack(html, headers = {}) {
  const frameworks = STACK_SIGNATURES.filter(([, pattern]) => pattern.test(html)).map(([name]) => name);
  const hosting = [];
  if (headers['cf-ray'] || /cloudflare/i.test(headers.server ?? '')) hosting.push('Cloudflare (proxy ou Pages/Workers)');
  if (headers['x-vercel-id'] || /vercel/i.test(headers.server ?? '')) hosting.push('Vercel');
  if (headers['x-nf-request-id'] || /netlify/i.test(headers.server ?? '')) hosting.push('Netlify');
  if (/github\.com/i.test(headers.server ?? '')) hosting.push('GitHub Pages');
  if (headers['x-powered-by']) hosting.push(`x-powered-by: ${headers['x-powered-by']}`);
  if (headers.server && !hosting.length) hosting.push(`server: ${headers.server}`);
  return { frameworks: frameworks.length ? frameworks : ['não identificado'], hosting: hosting.length ? hosting : ['não identificado'] };
}
...
async function staticAudit() {
  ...
  return {
    ...
    stack: detectStack(html, Object.fromEntries(response.headers)),
...
async function browserAudit({ chromium }) {
  ...
  const documentResponse = await page.goto(landing, { waitUntil: 'load', timeout: 60_000 });
  const documentHeaders = documentResponse ? await documentResponse.allHeaders() : {};
  const documentHtml = documentResponse ? await documentResponse.text().catch(() => '') : '';
  ...
  return {
    ...
    url: landing,
    stack: detectStack(documentHtml || html, documentHeaders),
    tagsInHtml: scanText(html, HTML_SIGNATURES),
    globals,
...
function printMarkdown(report) {
  ...
  section('Stack detectada (orienta os templates: references/stacks.md)', report.stack);
